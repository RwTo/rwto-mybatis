...
public class PooledDataSource implements DataSource {
    ...
    //不同的 url + username + password 会生成不同的 expectedConnectionTypeCode
        ...
        synchronized (state) {
            ...
            if (connection.isValid()) {
                ...
                if (state.idleConnections.size() < poolMaximumIdleConnections && connection.getConnectionTypeCode() == expectedConnectionTypeCode) {
                    ...
                    // 如果不是自动提交模式，为防止有没有提交的事务，需要先回滚
                    if (!connection.getRealConnection().getAutoCommit()) {
                        connection.getRealConnection().rollback();
                    }
                    /** 为什么要重新new一个对象，再放回池子： 为了保证池子中的对象是干净的，防止老对象还在调用方手里，继续执行操作 */
                    // 实例化一个新的DB连接，加入到idle列表（realConnection还是同一个）
                    PooledConnection newConnection = new PooledConnection(connection.getRealConnection(), this);
                    state.idleConnections.add(newConnection);
                    newConnection.setCreatedTimestamp(connection.getCreatedTimestamp());
                    newConnection.setLastUsedTimestamp(connection.getLastUsedTimestamp());
                    // 历史连接无效化, 就算调用方还持有老连接的引用，依然不能进行其他连接操作
